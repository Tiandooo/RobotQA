...
# 启动任务
start_response = requests.post(f"{BASE_URL}/data-qa/start", json=task_params)
...
time.sleep(0.1)
# 查询任务进度（无需停顿）
progress_response = requests.get(f"{BASE_URL}/data-qa/progress", params={"task_id": task_id})
...
time.sleep(0.1)
# 终止任务
stop_response = requests.post(f"{BASE_URL}/data-qa/stop", params={"task_id": task_id})
...
# 再次查询任务进度
progress_response_after_stop = requests.get(f"{BASE_URL}/data-qa/progress", params={"task_id": task_id})
