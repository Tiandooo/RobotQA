...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("data_format_check.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("DataFormatChecker")

class DataFormatChecker:
    """
    数据格式检查器，用于检测图像、视频和运动学数据的格式是否符合标准要求
    """
    ...
    def _scan_directory_structure(self) -> None:
        ...
        """
        logger.info(f"扫描数据集目录结构: {self.dataset_root}")
        ...
        # 记录扫描结果
        logger.info(f"扫描完成，找到 {sum(len(views) for views in self.file_paths['images'].values())} 个图像视角目录")
        logger.info(f"扫描完成，找到 {len(self.file_paths['depth'])} 个深度图目录")
        logger.info(f"扫描完成，找到 {len(self.file_paths['videos'])} 个视频目录")
        logger.info(f"扫描完成，找到 {len(self.file_paths['kinematic'])} 个运动学数据目录")
        logger.info(f"扫描完成，找到 {len(self.file_paths['dynamic'])} 个动力学数据目录")
    ...
    def check_all(self) -> Dict[str, Any]:
        ...
        返回:
            包含所有检查结果的字典
        """
        logger.info(f"开始检查数据集: {self.dataset_root}")
        # 检查图像数据
    ...
    def _check_single_image(self, img_path: str, episode: str, view: str) -> None:
        ...
        try:
            ...
            if not is_uint8:
                logger.warning(f"图像数据类型不符合要求: {img_path}, 类型: {dtype}, 应为: uint8")
        except Exception as e:
            logger.error(f"检查图像文件时出错: {img_path}, 错误: {str(e)}")
            self.results["image_checks"].append({
                "file": img_path,
                "episode": episode,
                "view": view,
    ...
    def _check_video_data(self) -> None:
        ...
        检查视频数据格式 (如果有)
        """
        logger.info("开始检查视频数据格式...")
        # 如果没有找到视频目录，记录信息并返回
        if not self.file_paths["videos"]:
            logger.info("未找到视频数据目录")
            return
    ...
    def _check_single_video(self, video_path: str) -> None:
        ...
        try:
            ...
            if not all_uint8:
                logger.warning(f"视频帧数据类型不符合要求: {video_path}, 应为: uint8")
            if not all_consistent:
                logger.warning(f"视频帧数据类型不一致: {video_path}")
        except Exception as e:
            logger.error(f"检查视频文件时出错: {video_path}, 错误: {str(e)}")
            self.results["video_checks"].append({
                "file": video_path,
                "status": "error",
                "message": f"检查时出错: {str(e)}"
    ...
    def _check_kinematic_data(self) -> None:
        ...
        检查运动学数据格式
        """
        logger.info("开始检查运动学数据格式...")
        
        # 如果没有找到运动学数据，记录信息并返回
        if not self.file_paths["kinematic"]:
            logger.warning("未找到运动学数据文件")
            return
    ...
    def _check_dynamic_data(self) -> None:
        ...
        检查动力学数据格式
        """
        logger.info("开始检查动力学数据格式...")
        
        # 如果没有找到动力学数据，记录信息并返回
        if not self.file_paths["dynamic"]:
            logger.warning("未找到动力学数据文件")
            return
    ...
    def _check_single_csv(self, csv_path: str, episode: str, is_kinematic: bool) -> None:
        ...
        try:
            ...
            if not all_float64:
                logger.warning(f"{'运动学' if is_kinematic else '动力学'}数据类型不符合要求: {csv_path}")
                logger.warning(f"不符合要求的列: {non_float64_columns}")
        except Exception as e:
            logger.error(f"检查CSV文件时出错: {csv_path}, 错误: {str(e)}")
            result = {
                "file": csv_path,
                "episode": episode,
                "status": "error",
            ...
            for view, img_paths in views.items():
                for img_path in img_paths:
                    ...
                    self.results["naming_checks"].append({
                        ...
                        "message": "图像文件名符合规则" if match else f"图像文件名不符合规则: {filename}"
                    })
        ...
        for episode, img_paths in self.file_paths["depth"].items():
            for img_path in img_paths:
                ...
                self.results["naming_checks"].append({
                    ...
                    "status": "pass" if match else "fail",
                    "expected_pattern": pattern,
                    "message": "深度图文件名符合规则" if match else f"深度图文件名不符合规则: {filename}"
                })
                if not re.match(pattern, filename):
                    self.results["naming_checks"].append({
                        "file": img_path,
        ...
        for episode, video_paths in self.file_paths["videos"].items():
            for video_path in video_paths:
                ...
                self.results["naming_checks"].append({
                    ...
                    "status": "pass" if match else "fail",
                    "expected_pattern": pattern,
                    "message": "视频文件名符合规则" if match else f"视频文件名不符合规则: {filename}"
                })
                if not re.match(pattern, filename):
                    self.results["naming_checks"].append({
                        "file": video_path,
        ...
        for episode, csv_paths in self.file_paths["kinematic"].items():
            for csv_path in csv_paths:
                ...
                self.results["naming_checks"].append({
                    ...
                    "status": "pass" if match else "fail",
                    "expected_pattern": pattern,
                    "message": "运动学数据文件名符合规则" if match else f"运动学数据文件名不符合规则: {filename}"
                })
                if not re.match(pattern, filename):
                    self.results["naming_checks"].append({
                        "file": csv_path,
        ...
        for episode, csv_paths in self.file_paths["dynamic"].items():
            for csv_path in csv_paths:
                ...
                self.results["naming_checks"].append({
                    ...
                    "status": "pass" if match else "fail",
                    "expected_pattern": pattern,
                    "message": "动力学数据文件名符合规则" if match else f"动力学数据文件名不符合规则: {filename}"
                })
                if not re.match(pattern, filename):
                    self.results["naming_checks"].append({
                        "file": csv_path,
    ...
    def generate_report(self, output_path: str = "data_format_check_report.json") -> None:
        """
        生成检查报告
        参数:
            output_path: 输出报告的路径
        ...
        logger.info(f"检查报告已保存至: {output_path}")
        # 打印摘要
        logger.info("检查摘要:")
        logger.info(f"图像检查: 总计 {stats['image']['total']}, 通过 {stats['image']['pass']}, 失败 {stats['image']['fail']}, 错误 {stats['image']['error']}")
        logger.info(f"视频检查: 总计 {stats['video']['total']}, 通过 {stats['video']['pass']}, 失败 {stats['video']['fail']}, 错误 {stats['video']['error']}")
        logger.info(f"运动学数据检查: 总计 {stats['kinematic']['total']}, 通过 {stats['kinematic']['pass']}, 失败 {stats['kinematic']['fail']}, 错误 {stats['kinematic']['error']}")
        logger.info(f"动力学数据检查: 总计 {stats['dynamic']['total']}, 通过 {stats['dynamic']['pass']}, 失败 {stats['dynamic']['fail']}, 错误 {stats['dynamic']['error']}")
        logger.info(f"命名规则检查: 总计不符合规则 {stats['naming']['fail']}")
...
def main():
    ...
    """
    import argparse
    parser = argparse.ArgumentParser(description='检查数据集格式是否符合标准要求')
    parser.add_argument('dataset_root', type=str, help='数据集根目录路径')
    parser.add_argument('--output', type=str, default='data_format_check_report.json', help='输出报告路径')
